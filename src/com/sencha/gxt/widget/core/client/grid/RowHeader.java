...
public class RowHeader<C> extends CellComponent<C> {
	...
	public interface ColumnHeaderAppearance {
		...
		/**
		 * Returns the icon to use for the "Columns" (column selection) header
		 * menu item.
		 * 
	...
	public class GridSplitBar extends Component {
		...
		protected void onDragStart(DragStartEvent e) {
			...
			dragging = true;
			getElement().getStyle()
					.setProperty("borderLeft", "1px solid black");
			...
			for (int i = 0, len = cols; i < len; i++) {
				...
				if (hd != null) {
					...
					if (startX > rr.getRight() - 5
							&& startX < rr.getRight() + 5) {
			...
			if (colIndex > -1) {
				...
				int maxx = (XElement.as(container.getElement()).getX() + XElement
						.as(container.getElement()).getWidth(false))
		...
		protected void onMouseMove(Head header, Event event) {
			...
			if (x - r.getLeft() <= hw && before != -1 && cm.isResizable(before)
					&& !cm.isFixed(before)) {
				bar.getElement().setVisibility(true);
				getElement().setX(r.getLeft() - (hw / 2));
				ss.setProperty("cursor", GXT.isSafari() ? "e-resize"
						: "col-resize");
			} else if (r.getRight() - x <= hw && cm.isResizable(activeHdIndex)
					&& !cm.isFixed(activeHdIndex)) {
				bar.getElement().setVisibility(true);
				getElement().setX(r.getRight() - (hw / 2));
				ss.setProperty("cursor", GXT.isSafari() ? "w-resize"
						: "col-resize");
			} else {
				bar.getElement().setVisibility(false);
		...
		public Group(HeaderGroupConfig config) {
			...
			if (config.getWidget() != null) {
				...
				getElement().setInnerHTML(
						config.getHtml() != null ? config.getHtml().asString()
								: SafeHtmlUtils.fromString("").asString());
			}
		}
	...
	public class Head extends Component {
		...
		protected ColumnConfig<C, ?> config;
		...
		public Head(ColumnConfig column) {
			...
			String s = config.getColumnClassSuffix() == null ? ""
					: " x-grid-hd-" + config.getColumnClassSuffix();
		...
		public void updateWidth(int width) {
			if (!config.isHidden()) {
				...
				getElement().setWidth(
						width - td.getFrameWidth(Side.LEFT, Side.RIGHT), true);
			}
		}
	}

	protected GridSplitBar bar;
	protected ColumnModel<C> cm;
	 ...
	 *            the column model
	 */
	public RowHeader(Widget container, ColumnModel<C> cm, Cell<C> cell) {
		this(container, cm, GWT
	 ...
	 */
	public RowHeader(Widget container, ColumnModel<C> cm,
			ColumnHeaderAppearance appearance, Cell<C> cell) {
		super(cell);
		...
		getElement().selectNode(appearance.columnsWrapSelector()).appendChild(
				table.getElement());
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < columns; j++) {
				...
				for (int k = mark; k < (mark + colspan); k++) {
					ColumnConfig<C, ?> c = cm.getColumn(k);
					if (c.isHidden()) {
						continue;
					}
				...
				int adj = cell.<XElement> cast().getFrameWidth(Side.LEFT,
						Side.RIGHT);
	...
	protected void cleanCells() {
		NodeList<Element> tds = DomQuery.select("tr." + styles.headRow()
				+ " > td", table.getElement());
	...
	public Head getHead(int column) {
		return (column > -1 && column < heads.size()) ? heads.get(column)
				: null;
	}
	...
	public void refresh() {
		...
		for (int i = 0; i < cols; i++) {
			...
			ColumnConfig<C, ?> cc = cm.getColumn(i);
			String s = cc.getColumnClassSuffix() == null ? "" : " x-grid-hd-"
					+ cc.getColumnClassSuffix();
			...
			if (align != null) {
				...
				if (align == HasHorizontalAlignment.ALIGN_RIGHT) {
					table.getCellFormatter().getElement(row, i)
		...
		if (container instanceof Grid) {
			...
			Grid<C> grid = (Grid<C>) container;
			if (grid.getView().isRemoteSort()) {
				List<? extends SortInfo> sortInfos = grid.getLoader()
						.getSortInfo();
				if (sortInfos.size() > 0) {
					SortInfo sortInfo = sortInfos.get(0);
					String sortField = sortInfo.getSortField();
					if (sortField != null && !"".equals(sortField)) {
						ColumnConfig<C, ?> column = cm
								.findColumnConfig(sortField);
						if (column != null) {
				...
				List<StoreSortInfo<C>> sortInfos = grid.getStore()
						.getSortInfo();
				if (sortInfos.size() > 0) {
					StoreSortInfo<C> sortInfo = sortInfos.get(0);
					if (sortInfo != null && sortInfo.getValueProvider() != null) {
						ColumnConfig<C, ?> column = grid.getColumnModel()
								.findColumnConfig(sortInfo.getPath());
						if (column != null) {
							updateSortIcon(grid.getColumnModel()
									.indexOf(column), sortInfo.getDirection());
						}
					}
				}
			}
	...
	public void showColumnMenu(final int column) {
		...
		if (menu != null) {
			...
			menu.show(h.getTrigger(), new AnchorAlignment(Anchor.TOP_LEFT,
	...
	public void updateTotalWidth(int offset, int width) {
		if (offset != -1)
			table.getElement().getParentElement().getStyle()
					.setWidth(++offset, Unit.PX);
